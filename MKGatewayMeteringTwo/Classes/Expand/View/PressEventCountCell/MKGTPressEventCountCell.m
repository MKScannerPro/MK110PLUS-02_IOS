...
@implementation MKGTPressEventCountCellModel
@end

@interface MKGTPressEventCountCell ()
...
@implementation MKGTPressEventCountCell

+ (MKGTPressEventCountCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTPressEventCountCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTPressEventCountCellIdenty"];
    if (!cell) {
        cell = [[MKGTPressEventCountCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTPressEventCountCellIdenty"];
    }
    return cell;
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_pressEventCountCell_clearButtonPressed:)]) {
        [self.delegate gt_pressEventCountCell_clearButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTPressEventCountCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTPressEventCountCellModel.class]) {
        return;
    }

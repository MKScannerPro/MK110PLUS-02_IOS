...
@implementation MKGTBleWifiSettingsCertCellModel
@end

@interface MKGTBleWifiSettingsCertCell ()
...
@implementation MKGTBleWifiSettingsCertCell

+ (MKGTBleWifiSettingsCertCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTBleWifiSettingsCertCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTBleWifiSettingsCertCellIdenty"];
    if (!cell) {
        cell = [[MKGTBleWifiSettingsCertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTBleWifiSettingsCertCellIdenty"];
    }
    return cell;
}
...
- (void)certificateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bleWifiSettingsCertPressed:)]) {
        [self.delegate gt_bleWifiSettingsCertPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTBleWifiSettingsCertCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTBleWifiSettingsCertCellModel.class]) {
        return;
    }
...
- (UIButton *)certificateButton {
    if (!_certificateButton) {
        ...
        [_certificateButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleWifiSettingsCertCell", @"gt_config_certAddIcon.png") forState:UIControlStateNormal];

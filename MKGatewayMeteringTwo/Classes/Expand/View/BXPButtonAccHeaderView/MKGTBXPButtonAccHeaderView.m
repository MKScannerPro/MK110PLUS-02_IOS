...
@interface MKGTBXPButtonAccHeaderView ()
...
@implementation MKGTBXPButtonAccHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bxpButtonAccHeaderView_syncButtonPressed:)]) {
        [self.delegate gt_bxpButtonAccHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bxpButtonAccHeaderView_exportButtonPressed)]) {
        [self.delegate gt_bxpButtonAccHeaderView_exportButtonPressed];
    }
}
...
- (void)updateSyncStatus:(BOOL)isOn {
    [self.syncIcon.layer removeAnimationForKey:@"mk_gt_syncAnimation"];
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPButtonAccHeaderView", @"gt_threeAxisAcceLoadingIcon.png");
    }
    return _syncIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPButtonAccHeaderView", @"gt_slotExportEnableIcon.png") forState:UIControlStateNormal];

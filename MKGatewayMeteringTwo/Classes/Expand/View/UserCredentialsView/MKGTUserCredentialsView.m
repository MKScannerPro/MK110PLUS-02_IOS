...
@implementation MKGTUserCredentialsViewModel
@end

@interface MKGTUserCredentialsView ()
...
@implementation MKGTUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKGTUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gt_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate gt_mqtt_userCredentials_userNameChanged:text];
            }
        };
        _userNameTextField.maxLength = 128;
        _userNameTextField.placeholder = @"0-128 Characters";
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gt_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate gt_mqtt_userCredentials_passwordChanged:text];
            }
        };
        _passwordTextField.maxLength = 128;
        _passwordTextField.placeholder = @"0-128 Characters";

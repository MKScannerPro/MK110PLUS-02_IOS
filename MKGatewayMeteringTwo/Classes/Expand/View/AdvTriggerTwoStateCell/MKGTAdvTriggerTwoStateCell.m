...
@implementation MKGTAdvTriggerTwoStateCellModel
@end

@interface MKGTAdvTriggerTwoStateCell ()
...
@implementation MKGTAdvTriggerTwoStateCell

+ (MKGTAdvTriggerTwoStateCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTAdvTriggerTwoStateCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTAdvTriggerTwoStateCellIdenty"];
    if (!cell) {
        cell = [[MKGTAdvTriggerTwoStateCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTAdvTriggerTwoStateCellIdenty"];
    }
    return cell;
}
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gt_advNormalCell_setPressed:beforeInterval:beforeTxPower:afterInterval:afterTxPower:)]) {
        [self.delegate gt_advNormalCell_setPressed:self.dataModel.index
...
- (void)txPowerButtonPressed:(UIButton *)btn {
    ...
    [pickView showPickViewWithDataList:self.txPowerList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_advNormalCell_txPowerChanged:before:txPower:)]) {
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTAdvTriggerTwoStateCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTAdvTriggerTwoStateCellModel.class]) {
        return;
    }

...
@implementation MKGTAdvNormalCellModel
@end

@interface MKGTAdvNormalCell ()
...
@implementation MKGTAdvNormalCell

+ (MKGTAdvNormalCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTAdvNormalCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTAdvNormalCellIdenty"];
    if (!cell) {
        cell = [[MKGTAdvNormalCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTAdvNormalCellIdenty"];
    }
    return cell;
}
...
- (void)setButtonPressed {
    NSString *txPower = self.txPowerBtn.titleLabel.text;
    txPower = [txPower stringByReplacingOccurrencesOfString:@"dBm" withString:@""];
    
    if ([self.delegate respondsToSelector:@selector(gt_advNormalCell_setPressed:interval:txPower:)]) {
...
#pragma mark - setter
- (void)setDataModel:(MKGTAdvNormalCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTAdvNormalCellModel.class]) {
        return;
    }
...
- (UILabel *)normalAdvLabel {
    if (!_normalAdvLabel) {
        _normalAdvLabel = [self loadLabelWithMsg:@"Normal ADV"];
    }

...
@implementation MKGTScanPageCell

+ (MKGTScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKGTScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKGTScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTScanPageModel.class]) {
        return;
    }

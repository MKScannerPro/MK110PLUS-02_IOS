...
static NSString *const localPasswordKey = @"mk_gt_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKGTScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_gt_centralManagerScanDelegate>
...
@implementation MKGTScanPageController
...
- (void)rightButtonMethod {
    if ([MKGTCentralManager shared].centralStatus != mk_gt_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKGTScanPageCell *cell = [MKGTScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_gt_centralManagerScanDelegate

- (void)mk_gt_receiveDevice:(NSDictionary *)deviceModel {
    MKGTScanPageModel *dataModel = [MKGTScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKGTCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKGTScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKGTScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKGTCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKGTScanPageModel *)deviecModel {
    ...
    [[MKGTCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTScanPageController", @"gt_scanRefresh.png");
    }
    return _refreshIcon;
}

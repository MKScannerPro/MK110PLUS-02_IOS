...
MKGatewayDeviceModelDelegate>

/// 没有添加设备的时候显示
@property (nonatomic, strong)MKGatewayAddDeviceView *addView;
...
@property (nonatomic, strong)MKEasyShowView *loadingView;
...
@implementation MKGTDeviceListController

- (void)dealloc {
    NSLog(@"MKGTDeviceListController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKGTMQTTDataManager shared] clearAllSubscriptions];
...
- (void)rightButtonMethod {
    MKGTServerForAppController *vc = [[MKGTServerForAppController alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (deviceModel.onLineState != MKGatewayDeviceModelStateOnline) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGTDeviceListCell *cell = [MKGTDeviceListCell initCellWithTableView:tableView];
...
#pragma mark - MKGTDeviceListCellDelegate
/**
 删除
...
#pragma mark - MKGatewayDeviceModelDelegate
/// 当前设备离线
/// @param deviceID 当前设备的deviceID
- (void)mk_deviceOfflineWithMacAddress:(NSString *)macAddress {
    [self deviceModelOnlineStateChanged:MKGatewayDeviceModelStateOffline macAddress:macAddress];
}
...
- (void)receiveNewDevice:(NSNotification *)note {
    ...
    MKGTDeviceModel *receiveModel = user[@"deviceModel"];
    MKGTDeviceListModel *deviceModel = [[MKGTDeviceListModel alloc] init];
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKGTDeviceListModel *model = self.dataList[i];
...
- (void)receiveDeviceOnlineState:(NSNotification *)note {
    ...
    [self deviceModelOnlineStateChanged:MKGatewayDeviceModelStateOnline macAddress:user[@"macAddress"]];
}
...
- (void)receiveDeleteDevice:(NSNotification *)note {
    ...
    MKGTDeviceListModel *deviceModel = nil;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKGTDeviceListModel *model = self.dataList[i];
    ...
    [[MKGTMQTTDataManager shared] unsubscriptions:unSubTopicList];
...
- (void)receiveDeviceModifyMQTTServer:(NSNotification *)note {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKGTDeviceModel *model = self.dataList[i];
        if ([model.macAddress isEqualToString:user[@"macAddress"]]) {
            ...
            model.onLineState = MKGatewayDeviceModelStateOffline;
...
- (void)resubTopics:(NSArray *)subTopicList {
    [self loadMainViews];
    [[MKGTMQTTDataManager shared] subscriptions:subTopicList];
}

/// 当前MQTT服务器连接状态发生改变
- (void)serverManagerStateChanged {
    if ([MKGTMQTTDataManager shared].state == MKGTMQTTSessionManagerStateConnecting) {
    ...
    if ([MKGTMQTTDataManager shared].state == MKGTMQTTSessionManagerStateError) {
        [self.loadingView hidden];
...
- (void)receiveDeviceLwtMessage:(NSNotification *)note {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        ...
        if ([deviceModel.macAddress isEqualToString:macAddress]) {
            deviceModel.onLineState = MKGatewayDeviceModelStateOffline;
            break;
        }
    }
    [self needRefreshList];
...
- (void)reloadDeviceTopics {
    ...
    NSMutableArray *topicList = [NSMutableArray array];
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKGTDeviceModel *deviceModel = self.dataList[i];
        [topicList addObject:[deviceModel currentPublishedTopic]];
    }
    [[MKGTMQTTDataManager shared] subscriptions:topicList];
}
...
- (void)readDataFromDatabase {
    ...
    [MKGatewayDatabaseManager readLocalDeviceWithDatabaseName:@"GTDeviceDB" tableName:@"GTDeviceTable" sucBlock:^(NSArray<NSDictionary *> * _Nonnull deviceList) {
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < deviceList.count; i ++) {
            MKGTDeviceModel *device = [MKGTDeviceModel mk_modelWithJSON:deviceList[i]];
            if (device) {
                [tempList addObject:device];
            }
        }
        [[MKHudManager share] hide];
        [self loadTopics:tempList];
...
- (void)loadTopics:(NSArray <MKGTDeviceModel *>*)deviceList {
    NSMutableArray *topicList = [NSMutableArray array];
    for (NSInteger i = 0; i < deviceList.count; i ++) {
        MKGTDeviceModel *tempModel = deviceList[i];
        MKGTDeviceListModel *deviceModel = [[MKGTDeviceListModel alloc] init];
...
- (void)deviceModelOnlineStateChanged:(MKGatewayDeviceModelState)state macAddress:(NSString *)macAddress {
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKGTDeviceListModel *deviceModel = self.dataList[i];
        if ([deviceModel.macAddress isEqualToString:macAddress]) {
            deviceModel.onLineState = state;
            if (state == MKGatewayDeviceModelStateOnline) {
                //在线状态开启监听
                [deviceModel startStateMonitoringTimer];
            }
            break;
...
- (void)removeDeviceFromLocal:(NSInteger)index {
    MKGTDeviceListModel *deviceModel = self.dataList[index];
    [[MKHudManager share] showHUDWithTitle:@"Delete..." inView:self.view isPenetration:NO];
    [MKGatewayDatabaseManager deleteWithDatabaseName:@"GTDeviceDB" tableName:@"GTDeviceTable" macAddress:deviceModel.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.dataList removeObject:deviceModel];
        [[MKGTMQTTDataManager shared] unsubscriptions:@[[deviceModel currentPublishedTopic]]];
        [self loadMainViews];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveNewDevice:)
                                                 name:@"mk_gt_addNewDeviceSuccessNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeleteDevice:)
                                                 name:@"mk_gt_deleteDeviceNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceModifyMQTTServer:)
                                                 name:@"mk_gt_deviceModifyMQTTServerSuccessNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(serverManagerStateChanged)
                                                 name:MKGTMQTTSessionManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceNetworkState:)
                                                 name:MKGTReceiveDeviceNetStateNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reloadDeviceTopics)
                                                 name:@"mk_gt_needReloadTopicsNotification"
                                               object:nil];
}
...
- (void)loadSubViews {
    self.defaultTitle = @"MKScannerPro";
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTDeviceListController", @"gt_menuIcon.png") forState:UIControlStateNormal];
...
- (MKGatewayAddDeviceView *)addView {
    if (!_addView) {
        _addView = [[MKGatewayAddDeviceView alloc] init];
    }
    return _addView;
}

- (MKEasyShowView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[MKEasyShowView alloc] init];
    }
    return _loadingView;
}

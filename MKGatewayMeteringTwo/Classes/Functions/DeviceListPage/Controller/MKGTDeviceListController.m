...
- (void)syncButtonPressed {
    ...
    if (ValidStr([MKGTUserLoginManager shared].password)) {
        //已经登陆过
        [self login:[MKGTUserLoginManager shared].isHome username:[MKGTUserLoginManager shared].username password:[MKGTUserLoginManager shared].password];
        return;
    }
    MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
    viewModel.account = [MKGTUserLoginManager shared].username;
    viewModel.isHome = [MKGTUserLoginManager shared].isHome;
    viewModel.password = [MKGTUserLoginManager shared].password;
...
- (void)readDataFromDatabase {
    ...
    [MKGTDeviceDatabaseManager readLocalDeviceWithSucBlock:^(NSArray<MKGTDeviceModel *> * _Nonnull deviceList) {
...
- (void)removeDeviceFromLocal:(NSInteger)index {
    ...
    [MKGTDeviceDatabaseManager deleteDeviceWithMacAddress:deviceModel.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.dataList removeObject:deviceModel];
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKNormalService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [[MKGTUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
        MKGTSyncDeviceController *vc = [[MKGTSyncDeviceController alloc] init];

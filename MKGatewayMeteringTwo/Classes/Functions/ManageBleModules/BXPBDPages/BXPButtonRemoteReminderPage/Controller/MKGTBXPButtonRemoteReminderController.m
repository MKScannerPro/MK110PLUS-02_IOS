//
//  MKGTBXPButtonRemoteReminderController.m
...
@interface MKGTBXPButtonRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPButtonRemoteReminderModel *dataModel;

@end

@implementation MKGTBXPButtonRemoteReminderController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKGTMQTTInterface gt_bxpBtnLedRemoteReminderWithBleMac:self.bleMac blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)reminderBuzzer {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnBuzzerRemoteReminderWithBleMac:self.bleMac ringTime:[self.dataModel.ringingTime integerValue] ringInterval:[self.dataModel.ringingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (MKGTBXPButtonRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPButtonRemoteReminderModel alloc] init];
    }
    return _dataModel;
}

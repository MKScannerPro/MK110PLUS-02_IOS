//
//  MKGTBXPButtonAdvParamsController.m
...
@interface MKGTBXPButtonAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPButtonAdvParamsModel *dataModel;

@end

@implementation MKGTBXPButtonAdvParamsController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonAdvParamsController销毁");
}
...
- (void)gt_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKGTMQTTInterface gt_bxpBtnConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTBXPButtonAdvParamsControllerCell"];
    }
...
- (MKGTBXPButtonAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPButtonAdvParamsModel alloc] init];
    }
    return _dataModel;
}

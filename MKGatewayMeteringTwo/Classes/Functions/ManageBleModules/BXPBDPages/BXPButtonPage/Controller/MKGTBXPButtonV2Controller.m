...
@interface MKGTBXPButtonV2Controller ()<UITableViewDelegate,
...
@implementation MKGTBXPButtonV2Controller

- (void)dealloc {
    NSLog(@"MKGTBXPButtonV2Controller销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 8 && indexPath.row == 0) {
        //Remote Reminder
        MKGTBXPButtonRemoteReminderController *vc = [[MKGTBXPButtonRemoteReminderController alloc] init];
    ...
    if (indexPath.section == 8 && indexPath.row == 1) {
        //Accelerometer data
        MKGTBXPButtonAccDataController *vc = [[MKGTBXPButtonAccDataController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 8 && indexPath.row == 2) {
        ...
        MKGTBXPButtonAdvParamsController *vc = [[MKGTBXPButtonAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)gt_buttonFirmwareCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        ...
        MKGTButtonDFUController *vc = [[MKGTButtonDFUController alloc] init];
        vc.bleMacAddress = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)gt_pressEventCountCell_clearButtonPressed:(NSInteger)index {
    ...
    [MKGTMQTTInterface gt_clearTriggerEventCount:index bleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)readDatasFromDevice {
    ...
    [MKGTMQTTInterface gt_readBXPButtonConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)dismissAlarmStatus {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_dismissBXPButtonAlarmStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnRemotePowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];

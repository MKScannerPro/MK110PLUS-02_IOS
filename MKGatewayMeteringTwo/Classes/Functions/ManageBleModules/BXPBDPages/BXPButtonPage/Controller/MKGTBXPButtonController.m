...
@interface MKGTBXPButtonController ()<UITableViewDelegate,
...
@implementation MKGTBXPButtonController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonController销毁");
...
- (void)sendLedReminder:(NSString *)interval duration:(NSString *)duration color:(NSInteger)color {
    ...
    [MKGTMQTTInterface gt_bxpBtnLedRemoteReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] blinkingTime:[duration integerValue] blinkingInterval:[interval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
- (void)sendBuzzerReminder:(NSString *)interval duration:(NSString *)duration {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnBuzzerRemoteReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] ringTime:[duration integerValue] ringInterval:[interval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"setup failed!"];
            return;

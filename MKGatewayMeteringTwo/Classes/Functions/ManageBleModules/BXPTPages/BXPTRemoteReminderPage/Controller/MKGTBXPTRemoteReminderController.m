//
//  MKGTBXPTRemoteReminderController.m
...
@interface MKGTBXPTRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPTRemoteReminderModel *dataModel;

@end

@implementation MKGTBXPTRemoteReminderController

- (void)dealloc {
    NSLog(@"MKGTBXPTRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKGTMQTTInterface gt_bxpBXPTLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKGTBXPTRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPTRemoteReminderModel alloc] init];
    }
    return _dataModel;
}

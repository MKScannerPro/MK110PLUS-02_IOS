...
@interface MKGTBXPTAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPTAccParamsModel *dataModel;

@end

@implementation MKGTBXPTAccParamsController

- (void)dealloc {
    NSLog(@"MKGTBXPTAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPTAccParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBXPTAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPTAccParamsModel alloc] init];
    }
    return _dataModel;
}

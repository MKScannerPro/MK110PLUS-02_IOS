...
@implementation MKGTManageBleDevicesCellModel
@end

@interface MKGTManageBleDevicesCell ()
...
@implementation MKGTManageBleDevicesCell

+ (MKGTManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKGTManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate gt_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTManageBleDevicesCellModel.class]) {
        return;
    }

//
//  MKGTManageBleDeviceSearchView.m
...
@interface MKGTManageBleDeviceSearchView ()<UIGestureRecognizerDelegate>
...
@implementation MKGTManageBleDeviceSearchView

- (void)dealloc{
    NSLog(@"MKGTManageBleDeviceSearchView销毁");
}
...
- (void)rssiValueChanged{
    [self.rssiValueLabel setText:[NSString stringWithFormat:@"%.fdBm",(-127 - self.slider.value)]];
}
...
- (void)doneButtonPressed{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        if (self.searchBlock) {
            self.searchBlock(SafeStr(self.nameTextField.text), SafeStr(self.macTextField.text),(-127 - [value integerValue]));
        }
        if (self.superview) {
            [self removeFromSuperview];
        }
...
+ (void)showSearchName:(NSString *)name
    ...
    MKGTManageBleDeviceSearchView *view = [[MKGTManageBleDeviceSearchView alloc] init];
...
- (UIImageView *)signalIcon{
    if (!_signalIcon) {
        _signalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(offset_X,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _signalIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTManageBleDeviceSearchView", @"gt_wifiSignalIcon.png");
    }
    return _signalIcon;
}
...
- (UIImageView *)graySignalIcon {
    if (!_graySignalIcon) {
        _graySignalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(backViewWidth - offset_X - signalIconWidth,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _graySignalIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTManageBleDeviceSearchView", @"gt_wifiGraySignalIcon.png");
    }

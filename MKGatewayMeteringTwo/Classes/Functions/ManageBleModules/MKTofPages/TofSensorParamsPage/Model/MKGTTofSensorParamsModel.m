...
@interface MKGTTofSensorParamsModel ()
...
@implementation MKGTTofSensorParamsModel
...
- (BOOL)readSensorParams:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_readMKTofSensorParamsWithBleMacAddress:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configSensorParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configMKTofSensorParamsWithInterval:[self.interval integerValue] sampleCount:[self.count integerValue] sampleTime:[self.time integerValue] bleMac:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_readMKTofRangingModeWithBleMacAddress:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configMKTofRangingMode:self.distanceMode bleMac:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

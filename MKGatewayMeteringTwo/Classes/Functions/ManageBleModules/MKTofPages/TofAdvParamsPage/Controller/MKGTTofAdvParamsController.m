...
@interface MKGTTofAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTTofAdvParamsModel *dataModel;

@end

@implementation MKGTTofAdvParamsController

- (void)dealloc {
    NSLog(@"MKGTTofAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTTofAdvParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTTofAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTTofAdvParamsModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTTofController ()<UITableViewDelegate,
...
@implementation MKGTTofController

- (void)dealloc {
    NSLog(@"MKGTTofController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Tof sensor data
        MKGTTofSensorDataController *vc = [[MKGTTofSensorDataController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Accelerometer data
        MKGTTofAccelerometerController *vc = [[MKGTTofAccelerometerController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        ...
        MKGTTofSensorParamsController *vc = [[MKGTTofSensorParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 3) {
        ...
        MKGTTofAdvParamsController *vc = [[MKGTTofAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKGTMQTTInterface gt_readMKTofConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpMKTofPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection3Datas {
    ...
    cellModel1.leftMsg = @"TOF sensor data";
    ...
    cellModel3.leftMsg = @"TOF sensor parameters";

...
@interface MKGTTofSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGTTofSensorDataController

- (void)dealloc {
    NSLog(@"MKGTTofSensorDataController销毁");
...
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_bxpMKTofNotifySensorDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)gt_bxpButtonAccHeaderView_exportButtonPressed {
    ...
    [mailComposer addAttachmentData:emailData
                           ...
                           fileName:@"MK Tof SensorDatas.txt"];
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"TOF sensor data";

...
@implementation MKGTButtonFirmwareCellModel
@end

@interface MKGTButtonFirmwareCell ()
...
@implementation MKGTButtonFirmwareCell

+ (MKGTButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKGTButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate gt_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTButtonFirmwareCellModel.class]) {
        return;
    }

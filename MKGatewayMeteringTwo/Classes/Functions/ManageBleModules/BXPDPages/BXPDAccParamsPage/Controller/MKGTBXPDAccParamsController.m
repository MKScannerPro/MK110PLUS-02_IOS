...
@interface MKGTBXPDAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPDAccParamsModel *dataModel;

@end

@implementation MKGTBXPDAccParamsController

- (void)dealloc {
    NSLog(@"MKGTBXPDAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPDAccParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBXPDAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPDAccParamsModel alloc] init];
    }
    return _dataModel;
}

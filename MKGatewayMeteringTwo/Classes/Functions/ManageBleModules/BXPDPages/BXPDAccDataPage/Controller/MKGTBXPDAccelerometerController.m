//
//  MKGTBXPDAccelerometerController.m
...
@interface MKGTBXPDAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGTBXPDAccelerometerController

- (void)dealloc {
    NSLog(@"MKGTBXPDAccelerometerController销毁");
...
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_bxpBXPDNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKGTReceiveBXPDAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }

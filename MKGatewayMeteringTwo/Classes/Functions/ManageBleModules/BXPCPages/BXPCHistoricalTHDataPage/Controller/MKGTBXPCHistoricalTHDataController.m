//
//  MKGTBXPCHistoricalTHDataController.m
...
@interface MKGTBXPCHistoricalTHDataController ()<MFMailComposeViewControllerDelegate,
MKGTBXPCHistoricalTHDataHeaderViewDelegate>

@property (nonatomic, strong)MKGTBXPCHistoricalTHDataHeaderView *headerView;
...
@implementation MKGTBXPCHistoricalTHDataController

- (void)dealloc {
    NSLog(@"MKGTBXPCHistoricalTHDataController销毁");
...
#pragma mark - MKGTBXPCHistoricalTHDataHeaderViewDelegate
- (void)gt_bxpcHistoricalHTDataHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBXPCNotifyHistoricalHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)gt_bxpcHistoricalHTDataHeaderView_deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBXPCDeleteHistoricalHTDataWithBleMac:self.bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
        self.textView.text = @"";
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKGTBXPCHistoricalTHDataHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKGTBXPCHistoricalTHDataHeaderView alloc] init];

//
//  MKGTBXPCHistoricalTHDataHeaderView.m
...
@interface MKGTBXPCHistoricalTHDataHeaderView ()
...
@implementation MKGTBXPCHistoricalTHDataHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bxpcHistoricalHTDataHeaderView_syncButtonPressed:)]) {
        [self.delegate gt_bxpcHistoricalHTDataHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bxpcHistoricalHTDataHeaderView_deleteButtonPressed)]) {
        [self.delegate gt_bxpcHistoricalHTDataHeaderView_deleteButtonPressed];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_bxpcHistoricalHTDataHeaderView_exportButtonPressed)]) {
        [self.delegate gt_bxpcHistoricalHTDataHeaderView_exportButtonPressed];
    }
}

#pragma mark - public method
- (void)updateSyncStatus:(BOOL)isOn {
    [self.synIcon.layer removeAnimationForKey:@"mk_gt_syncAnimation"];
    self.syncButton.selected = isOn;
    if (isOn) {
        [self.synIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                                   forKey:@"mk_gt_syncAnimation"];
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPCHistoricalTHDataHeaderView", @"gt_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPCHistoricalTHDataHeaderView", @"gt_slotExportEnableIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBXPCHistoricalTHDataHeaderView", @"gt_slotExportDeleteIcon.png") forState:UIControlStateNormal];

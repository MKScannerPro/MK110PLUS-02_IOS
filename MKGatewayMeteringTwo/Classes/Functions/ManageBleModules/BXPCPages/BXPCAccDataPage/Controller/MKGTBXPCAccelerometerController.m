//
//  MKGTBXPCAccelerometerController.m
...
@interface MKGTBXPCAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGTBXPCAccelerometerController

- (void)dealloc {
    NSLog(@"MKGTBXPCAccelerometerController销毁");
...
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_bxpBXPCNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKGTReceiveBXPCAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }

...
@interface MKGTBXPCTHDataSampleRateModel ()
...
- (BOOL)readTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_readBXPCTHDataSampleRateWithBleMacAddress:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_configBXPCSampleRate:[self.self.sampleRate integerValue] bleMac:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

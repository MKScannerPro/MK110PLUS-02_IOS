//
//  MKGTBXPButtonCRRemoteReminderController.m
...
@interface MKGTBXPButtonCRRemoteReminderController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGTRemoteReminderCellDelegate>
...
@property (nonatomic, strong)MKGTBXPButtonCRRemoteReminderModel *dataModel;

@end

@implementation MKGTBXPButtonCRRemoteReminderController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonCRRemoteReminderController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        //LED notification
        MKGTRemoteReminderCell *cell = [MKGTRemoteReminderCell initCellWithTableView:tableView];
...
#pragma mark - MKGTRemoteReminderCellDelegate
...
- (void)reminderLED {
    ...
    [MKGTMQTTInterface gt_bxpBtnCRLedRemoteReminderWithBleMac:self.bleMac blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)reminderBuzzer {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnCRBuzzerRemoteReminderWithBleMac:self.bleMac ringTime:[self.dataModel.ringingTime integerValue] ringInterval:[self.dataModel.ringingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)reminderVibration {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnCRVibratingRemoteReminderWithBleMac:self.bleMac vibratingTime:[self.dataModel.vibrationTime integerValue] vibratingInterval:[self.dataModel.vibrationInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection0Datas {
    MKGTRemoteReminderCellModel *cellModel = [[MKGTRemoteReminderCellModel alloc] init];
...
- (MKGTBXPButtonCRRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPButtonCRRemoteReminderModel alloc] init];
    }
    return _dataModel;
}

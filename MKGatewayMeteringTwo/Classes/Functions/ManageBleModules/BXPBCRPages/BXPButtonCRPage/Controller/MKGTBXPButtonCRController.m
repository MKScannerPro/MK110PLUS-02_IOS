...
@interface MKGTBXPButtonCRController ()<UITableViewDelegate,
UITableViewDataSource,
MKButtonMsgCellDelegate,
MKGTButtonFirmwareCellDelegate,
MKGTPressEventCountCellDelegate>
...
@implementation MKGTBXPButtonCRController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonCRController销毁");
...
- (void)leftButtonMethod {
    //用户点击左上角，则需要返回MKGTDeviceDataController
    [self popToViewControllerWithClassName:@"MKGTDeviceDataController"];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 7 && indexPath.row == 0) {
        //Remote Reminder
        MKGTBXPButtonCRRemoteReminderController *vc = [[MKGTBXPButtonCRRemoteReminderController alloc] init];
    ...
    if (indexPath.section == 7 && indexPath.row == 1) {
        //Alarm Event
        MKGTBXPButtonCRAlarmEventController *vc = [[MKGTBXPButtonCRAlarmEventController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 7 && indexPath.row == 2) {
        //Accelerometer data
        MKGTBXPButtonCRAccDataController *vc = [[MKGTBXPButtonCRAccDataController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 7 && indexPath.row == 3) {
        ...
        MKGTBXPButtonCRAdvParamsController *vc = [[MKGTBXPButtonCRAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGTButtonFirmwareCell *cell = [MKGTButtonFirmwareCell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 5) {
        MKGTPressEventCountCell *cell = [MKGTPressEventCountCell initCellWithTableView:tableView];
...
#pragma mark - MKGTButtonFirmwareCellDelegate
- (void)gt_buttonFirmwareCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKGTButtonDFUV2Controller *vc = [[MKGTButtonDFUV2Controller alloc] init];
...
#pragma mark - MKGTPressEventCountCellDelegate
- (void)gt_pressEventCountCell_clearButtonPressed:(NSInteger)index {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    
    [MKGTMQTTInterface gt_clearBXPButtonCREventCountWithType:index bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        MKGTPressEventCountCellModel *cellModel = self.section5List[index];
        cellModel.count = @"0";
...
- (void)readDatasFromDevice {
    ...
    [MKGTMQTTInterface gt_readBXPButtonCRConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)dismissAlarmStatus {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_dismissBXPButtonCRAlarmStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)disconnect {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_disconnectNormalBleDeviceWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBtnCRRemotePowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)updateStatusDatas {
    ...
    MKGTPressEventCountCellModel *cellModel2 = self.section5List[0];
    cellModel2.count = [NSString stringWithFormat:@"%@",self.bxpStatusDic[@"data"][@"single_alarm_num"]];
    
    MKGTPressEventCountCellModel *cellModel3 = self.section5List[1];
    cellModel3.count = [NSString stringWithFormat:@"%@",self.bxpStatusDic[@"data"][@"double_alarm_num"]];
    
    MKGTPressEventCountCellModel *cellModel4 = self.section5List[2];
...
- (void)powerOff {
    ...
    [alertView showAlertWithTitle:@""
                 ...
                 notificationName:@"mk_gt_needDismissAlert"];
    return;
}
...
- (void)loadSection1Datas {
    MKGTButtonFirmwareCellModel *cellModel = [[MKGTButtonFirmwareCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection5Datas {
    MKGTPressEventCountCellModel *cellModel1 = [[MKGTPressEventCountCellModel alloc] init];
    cellModel1.index = 0;
    cellModel1.msg = @"Single press event count";
    [self.section5List addObject:cellModel1];
    
    MKGTPressEventCountCellModel *cellModel2 = [[MKGTPressEventCountCellModel alloc] init];
    cellModel2.index = 1;
    cellModel2.msg = @"Double press event count";
    [self.section5List addObject:cellModel2];
    
    MKGTPressEventCountCellModel *cellModel3 = [[MKGTPressEventCountCellModel alloc] init];
    cellModel3.index = 2;

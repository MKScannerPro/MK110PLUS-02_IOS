//
//  MKGTBXPButtonCRAdvParamsController.m
...
@interface MKGTBXPButtonCRAdvParamsController ()<UITableViewDelegate,
UITableViewDataSource,
MKGTAdvNormalCellDelegate,
MKGTAdvTriggerCellDelegate,
MKGTAdvTriggerTwoStateCellDelegate>
...
@property (nonatomic, strong)MKGTBXPButtonCRAdvParamsModel *dataModel;

@end

@implementation MKGTBXPButtonCRAdvParamsController

- (void)dealloc {
    NSLog(@"MKGTBXPButtonCRAdvParamsController销毁");
}
...
#pragma mark - MKGTAdvNormalCellDelegate
- (void)gt_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKGTMQTTInterface gt_bxpBtnCRConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
#pragma mark - MKGTAdvTriggerCellDelegate
...
#pragma mark - MKGTAdvTriggerTwoStateCellDelegate
- (void)gt_advNormalCell_setPressed:(NSInteger)index
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTBXPButtonCRAdvParamsControllerCell"];
    }
    ...
    if (channelType == 0) {
        //正常广播
        MKGTAdvNormalCell *cell = [MKGTAdvNormalCell initCellWithTableView:self.tableView];
        cell.dataModel = dataList[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    if (channelType == 1) {
        //触发广播
        MKGTAdvTriggerCell *cell = [MKGTAdvTriggerCell initCellWithTableView:self.tableView];
        cell.dataModel = dataList[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    //触发前+触发后广播
    MKGTAdvTriggerTwoStateCell *cell = [MKGTAdvTriggerTwoStateCell initCellWithTableView:self.tableView];
    cell.dataModel = dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}
...
- (CGFloat)loadTableCellHeight:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKGTAdvNormalCellModel.class] || [obj isKindOfClass:MKGTAdvTriggerCellModel.class]) {
        return 150.f;
    }
    if ([obj isKindOfClass:MKGTAdvTriggerTwoStateCellModel.class]) {
        return 240.f;
    }
    return 0.0f;
}
...
- (NSObject *)loadObjectWithSection:(NSInteger)section {
    ...
    if (channelType == 0) {
        //正常广播
        MKGTAdvNormalCellModel *cellModel = [[MKGTAdvNormalCellModel alloc] init];
    ...
    if (channelType == 1) {
        //触发广播
        MKGTAdvTriggerCellModel *cellModel = [[MKGTAdvTriggerCellModel alloc] init];
        cellModel.index = section;
        cellModel.msg = channelMsg;
    ...
    //触发前+触发后广播
    MKGTAdvTriggerTwoStateCellModel *cellModel = [[MKGTAdvTriggerTwoStateCellModel alloc] init];
    cellModel.index = section;
    cellModel.msg = channelMsg;
...
- (MKGTBXPButtonCRAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPButtonCRAdvParamsModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTPirController ()<UITableViewDelegate,
...
@implementation MKGTPirController

- (void)dealloc {
    NSLog(@"MKGTPirController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //PIR sensor data
        MKGTPirSensorDataController *vc = [[MKGTPirSensorDataController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        ...
        MKGTPirSensorParamsController *vc = [[MKGTPirSensorParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        ...
        MKGTPirAdvParamsController *vc = [[MKGTPirAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKGTMQTTInterface gt_readMKPirConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpMKPirPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)updateStatusDatas {
    ...
    MKNormalTextCellModel *cellModel2 = self.section2List[4];
    cellModel2.rightMsg = [NSString stringWithFormat:@"%@%@",self.bxpStatusDic[@"data"][@"run_time"],@"s"];

...
@interface MKGTPirAdvParamsModel ()
...
@implementation MKGTPirAdvParamsModel
...
- (BOOL)readAdvParams:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_readMKPirAdvParamsWithBleMacAddress:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configMKPirAdvParamsWithInterval:[self.interval integerValue] txPower:self.txPower bleMac:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

...
@interface MKGTPirAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTPirAdvParamsModel *dataModel;

@end

@implementation MKGTPirAdvParamsController

- (void)dealloc {
    NSLog(@"MKGTPirAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTPirAdvParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTPirAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTPirAdvParamsModel alloc] init];
    }
    return _dataModel;
}

//
//  MKGTPirSensorParamsController.m
...
@interface MKGTPirSensorParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTPirSensorParamsModel *dataModel;

@end

@implementation MKGTPirSensorParamsController

- (void)dealloc {
    NSLog(@"MKGTPirSensorParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTPirSensorParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTPirSensorParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTPirSensorParamsModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTPirSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGTPirSensorDataController

- (void)dealloc {
    NSLog(@"MKGTPirSensorDataController销毁");
...
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_notifyMKPirSensorDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)gt_bxpButtonAccHeaderView_exportButtonPressed {
    ...
    [mailComposer addAttachmentData:emailData
                           ...
                           fileName:@"MK Pir SensorDatas.txt"];
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"PIR sensor data";

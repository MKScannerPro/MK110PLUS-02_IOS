...
@interface MKGTButtonDFUV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTButtonDFUV2Model *dataModel;
...
@implementation MKGTButtonDFUV2Controller

- (void)dealloc {
    NSLog(@"MKGTButtonDFUV2Controller销毁");
...
- (void)gotoLastPage {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:NSClassFromString(@"MKGTManageBleDevicesController")]) {
            //如果是从Manage BLE devices页面进来的
            [self popToViewControllerWithClassName:@"MKGTManageBleDevicesController"];
            break;
        }
        if ([vc isKindOfClass:NSClassFromString(@"MKGTDeviceDataController")]) {
            //如果是从扫描页面进来的
...
- (MKGTButtonDFUV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTButtonDFUV2Model alloc] init];

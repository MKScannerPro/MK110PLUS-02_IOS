//
//  MKGTNormalConnectedController.m
...
@interface MKGTNormalConnectedController ()<UITableViewDelegate,
UITableViewDataSource,
MKGTNormalConnectedCellDelegate>
...
@property (nonatomic, strong)MKGTConnectedDeviceWriteAlertView *writeAlert;

@end

@implementation MKGTNormalConnectedController

- (void)dealloc {
    NSLog(@"MKGTNormalConnectedController销毁");
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKGTNormalConnectedCellModel *cellModel = charList[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGTNormalConnectedCell *cell = [MKGTNormalConnectedCell initCellWithTableView:tableView];
    NSDictionary *dic = self.sectionList[indexPath.section];
    NSArray *charList = dic[@"charList"];
...
#pragma mark - MKGTNormalConnectedCellDelegate
- (void)gt_normalConnectedCell_writeButtonPressed:(NSInteger)section
                                              row:(NSInteger)row
                                       serverUUID:(NSString *)serverUUID
                                   characteristic:(NSString *)characteristic {
    @weakify(self);
    [self.writeAlert showAlertWithValue:@"" dismissNote:@"mk_gt_needDismissAlert" cancelAction:^{
...
- (void)gt_normalConnectedCell_readButtonPressed:(NSInteger)section
                                             row:(NSInteger)row
                                      serverUUID:(NSString *)serverUUID
                                  characteristic:(NSString *)characteristic {
...
- (void)gt_normalConnectedCell_notifyButtonPressed:(BOOL)notify
...
- (void)receiveDeviceDatas:(NSNotification *)note {
    ...
    MKGTNormalConnectedCellModel *cellModel = charList[row];
...
- (void)parseTableDatas {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        for (NSInteger j = 0; j < charCount; j ++) {
            ...
            MKGTNormalConnectedCellModel *cellModel = [[MKGTNormalConnectedCellModel alloc] init];
            cellModel.section = i;
            cellModel.row = j;
...
- (void)writeData:(NSString *)data serviceUUID:(NSString *)serverUUID characteristic:(NSString *)characteristic {
    ...
    [MKGTMQTTInterface gt_writeValueToDeviceWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] value:data serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)readDataWithServiceUUID:(NSString *)serverUUID
    ...
    [MKGTMQTTInterface gt_readCharacteristicValueWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
characteristic:(NSString *)characteristic
    ...
    [MKGTMQTTInterface gt_notifyCharacteristic:notify bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (MKGTConnectedDeviceWriteAlertView *)writeAlert {
    if (!_writeAlert) {
        _writeAlert = [[MKGTConnectedDeviceWriteAlertView alloc] init];
    }
    return _writeAlert;
}

...
@interface MKGTBXPSTHDataSampleRateModel ()
...
- (BOOL)readTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_readBXPSTHDataSampleRateWithBleMacAddress:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGTMQTTInterface gt_configBXPSSampleRate:[self.self.sampleRate integerValue] bleMac:bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

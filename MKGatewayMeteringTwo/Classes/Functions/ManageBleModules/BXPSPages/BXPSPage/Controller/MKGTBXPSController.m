...
@interface MKGTBXPSController ()<UITableViewDelegate,
...
@implementation MKGTBXPSController

- (void)dealloc {
    NSLog(@"MKGTBXPSController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Real time T&H data
        MKGTBXPSRealTimeTHDataController *vc = [[MKGTBXPSRealTimeTHDataController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Historical T&H data
        MKGTBXPSHistoricalTHDataController *vc = [[MKGTBXPSHistoricalTHDataController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        //Accelerometer data
        MKGTBXPSAccelerometerController *vc = [[MKGTBXPSAccelerometerController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 3) {
        //Hall sensor data
        MKGTBXPSHallCountController *vc = [[MKGTBXPSHallCountController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 4) {
        ...
        MKGTBXPSTHDataSampleRateController *vc = [[MKGTBXPSTHDataSampleRateController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 5) {
        ...
        MKGTBXPSAdvParamsController *vc = [[MKGTBXPSAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 6) {
        //Remote reminder
        MKGTBXPSRemoteReminderController *vc = [[MKGTBXPSRemoteReminderController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKGTMQTTInterface gt_readBXPSConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBXPSPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection2Datas {
    ...
    if ([self.deviceBleInfo[@"data"][@"axis_type"] integerValue] > 0) {
        [tempList addObject:@"ACC"];
    }
    
    if ([self.deviceBleInfo[@"data"][@"th_type"] integerValue] > 0) {
        [tempList addObject:@"TH"];
    }
    
    if ([self.deviceBleInfo[@"data"][@"light_type"] integerValue] > 0) {
        [tempList addObject:@"Light"];
    }
    
    if ([self.deviceBleInfo[@"data"][@"pir_type"] integerValue] > 0) {
        [tempList addObject:@"PIR"];
    }
    
    if ([self.deviceBleInfo[@"data"][@"tof_type"] integerValue] > 0) {
        [tempList addObject:@"TOF"];
    }

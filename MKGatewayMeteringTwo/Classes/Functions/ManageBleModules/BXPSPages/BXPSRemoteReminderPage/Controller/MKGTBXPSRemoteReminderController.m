//
//  MKGTBXPSRemoteReminderController.m
...
@interface MKGTBXPSRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBXPSRemoteReminderModel *dataModel;

@end

@implementation MKGTBXPSRemoteReminderController

- (void)dealloc {
    NSLog(@"MKGTBXPSRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKGTMQTTInterface gt_bxpBXPSLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKGTBXPSRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPSRemoteReminderModel alloc] init];
    }
    return _dataModel;
}

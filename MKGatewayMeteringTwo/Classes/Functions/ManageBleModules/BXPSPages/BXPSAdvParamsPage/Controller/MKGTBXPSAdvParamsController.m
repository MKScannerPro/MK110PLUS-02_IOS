...
@interface MKGTBXPSAdvParamsController ()<UITableViewDelegate,
UITableViewDataSource,
MKGTBXPSAdvNormalCellDelegate,
MKGTBXPSAdvTriggerCellDelegate,
MKGTBXPSAdvTriggerTwoStateCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKGTBXPSAdvParamsModel *dataModel;

@end

@implementation MKGTBXPSAdvParamsController

- (void)dealloc {
    NSLog(@"MKGTBXPSAdvParamsController销毁");
}
...
#pragma mark - MKGTBXPSAdvNormalCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
- (void)gt_BXPSAdvNormalCell_setPressed:(NSInteger)index
    ...
    [MKGTMQTTInterface gt_configBXPSAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
#pragma mark - MKGTBXPSAdvTriggerCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
///   - interval: 当前ADV interval
...
/*
 ...
 6
 */
- (void)gt_BXPSAdvTriggerCell_setPressed:(NSInteger)index
                                interval:(NSString *)interval
                                 txPower:(NSInteger)txPower {
    if (!ValidStr(interval) || [interval integerValue] < 1 || [interval integerValue] > 100) {
        [self.view showCentralToast:@"ADV interval Error"];
...
#pragma mark - MKGTBXPSAdvTriggerTwoStateCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKGTBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKGTBXPSAdvNormalCell *cell = [MKGTBXPSAdvNormalCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKGTBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKGTBXPSAdvTriggerCell *cell = [MKGTBXPSAdvTriggerCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKGTBXPSAdvTriggerTwoStateCellModel.class]) {
        //触发前+触发后广播
        MKGTBXPSAdvTriggerTwoStateCell *cell = [MKGTBXPSAdvTriggerTwoStateCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTBXPSAdvParamsControllerIdenty"];
}
...
- (CGFloat)loadTableCellHeight:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKGTBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKGTBXPSAdvNormalCellModel *tempModel = (MKGTBXPSAdvNormalCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKGTBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKGTBXPSAdvTriggerCellModel *tempModel = (MKGTBXPSAdvTriggerCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKGTBXPSAdvTriggerTwoStateCellModel.class]) {
        MKGTBXPSAdvTriggerTwoStateCellModel *tempModel = (MKGTBXPSAdvTriggerTwoStateCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    return 0.0f;
}
...
- (void)loadSectionDatas {
    for (NSInteger i = 0; i < self.dataModel.dataList.count; i ++) {
        ...
        if (!enable) {
            //通道关闭 No Data
            MKGTBXPSAdvNormalCellModel *cellModel = [[MKGTBXPSAdvNormalCellModel alloc] init];
            cellModel.slotIndex = channel;
            cellModel.slotType = MKGTBXPSAdvNormalCellSlotTypeNoData;
            ...
            if (channelType == 0) {
                ...
                MKGTBXPSAdvTriggerCellModel *cellModel = [[MKGTBXPSAdvTriggerCellModel alloc] init];
                cellModel.slotIndex = channel;
                ...
                NSDictionary *afterAdv = dic[@"trigger_after_adv"];
                MKGTBXPSAdvTriggerTwoStateCellModel *cellModel = [[MKGTBXPSAdvTriggerTwoStateCellModel alloc] init];
                cellModel.slotIndex = channel;
...
- (MKGTBXPSAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBXPSAdvParamsModel alloc] init];
    }
    return _dataModel;
}

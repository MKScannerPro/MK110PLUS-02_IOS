//
//  MKGTBXPSRealTimeTHDataController.m
...
@interface MKGTBXPSRealTimeTHDataController ()<MFMailComposeViewControllerDelegate,
MKGTBXPButtonAccHeaderViewDelegate>

@property (nonatomic, strong)MKGTBXPButtonAccHeaderView *headerView;
...
@implementation MKGTBXPSRealTimeTHDataController

- (void)dealloc {
    NSLog(@"MKGTBXPSRealTimeTHDataController销毁");
...
#pragma mark - MKGTBXPButtonAccHeaderViewDelegate
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_bxpBXPSNotifyRealTimeHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKGTReceiveBXPSRealTimeHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)gt_bxpButtonAccHeaderView_exportButtonPressed {
    ...
    [mailComposer addAttachmentData:emailData
                           ...
                           fileName:@"BXP-S RealTimeHTDatas.txt"];
...
#pragma mark - getter
- (MKGTBXPButtonAccHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKGTBXPButtonAccHeaderView alloc] init];

//
//  MKGTBXPSAccelerometerController.m
...
@interface MKGTBXPSAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGTBXPSAccelerometerController

- (void)dealloc {
    NSLog(@"MKGTBXPSAccelerometerController销毁");
...
- (void)gt_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_bxpBXPSNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKGTReceiveBXPSAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }

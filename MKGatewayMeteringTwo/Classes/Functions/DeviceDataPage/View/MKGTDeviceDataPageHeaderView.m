//
//  MKGTDeviceDataPageHeaderView.m
...
@implementation MKGTDeviceDataPageHeaderViewModel
@end

@interface MKGTDeviceDataPageManageBleButton : UIControl
...
@implementation MKGTDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewayMeteringTwo", @"MKGTDeviceDataPageManageBleButton", @"gt_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKGTDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKGTDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKGTDeviceDataPageHeaderView
...
- (void)layoutSubviews {
    ...
    [self.powerButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(-15.f);
        make.width.mas_equalTo(width);
        make.centerY.mas_equalTo(self.uploadButton.mas_centerY);
        make.height.mas_equalTo(40.f);
    }];
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_updateLoadButtonAction)]) {
        [self.delegate gt_updateLoadButtonAction];
    }
}

- (void)powerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_powerButtonAction)]) {
        [self.delegate gt_powerButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_scannerStatusChanged:)]) {
        [self.delegate gt_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_manageBleDeviceAction)]) {
        [self.delegate gt_manageBleDeviceAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTDeviceDataPageHeaderView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTDeviceDataPageHeaderView", @"gt_switchUnselectedIcon.png"));
...
- (UIButton *)powerButton {
    if (!_powerButton) {
        _powerButton = [MKCustomUIAdopter customButtonWithTitle:@"Power Metering"
                                                          target:self
...
- (MKGTDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKGTDeviceDataPageManageBleButton alloc] init];

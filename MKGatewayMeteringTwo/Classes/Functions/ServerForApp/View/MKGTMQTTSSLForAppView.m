...
@implementation MKGTMQTTSSLForAppViewModel
@end

@interface MKGTMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKGTMQTTSSLForAppView
...
- (void)layoutSubviews {
    ...
    CGSize clientSize = [NSString sizeWithText:self.dataModel.clientFileName
                                       andFont:MKFont(13.f)
                                    andMaxSize:CGSizeMake(self.frame.size.width - 2 * 15.f - 120.f -  2 * 10.f - 40.f, MAXFLOAT)];
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate gt_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate gt_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate gt_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate gt_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForAppView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForAppView", @"gt_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForAppView", @"gt_switchUnselectedIcon.png") forState:UIControlStateNormal];

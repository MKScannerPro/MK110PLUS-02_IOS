...
@property (nonatomic, strong)MKGTServerForAppModel *dataModel;

@property (nonatomic, strong)MKGTServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKGTServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKGTServerForAppController

- (void)dealloc {
    NSLog(@"MKGTServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKGTMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKGTMQTTDataManager shared] connect];
}
...
#pragma mark - MKGTServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)gt_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)gt_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)gt_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)gt_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)gt_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)gt_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)serverManagerStateChanged {
    ...
    if ([MKGTMQTTDataManager shared].state == MKGTMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKGTMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_gt_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKGTExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKGTMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKGTMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTServerForAppController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGTServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGTServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKGTServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGTServerConfigAppFooterViewModel alloc] init];
    }

...
@interface MKGTAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTAdvBeaconV2Model *dataModel;

@end

@implementation MKGTAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKGTAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTAdvBeaconV2Controller", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKGTSystemTimeCellDelegate,
...
@implementation MKGTSystemTimeController

- (void)dealloc {
    NSLog(@"MKGTSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKGTNTPServerController *vc = [[MKGTNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGTSystemTimeCell *cell = [MKGTSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKGTSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKGTMQTTInterface gt_readDeviceUTCTimeWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKGTMQTTInterface gt_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKGTSystemTimeCellModel *cellModel = [[MKGTSystemTimeCellModel alloc] init];
    cellModel.index = 0;

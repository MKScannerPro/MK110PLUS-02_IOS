...
@interface MKGTAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTAdvBeaconModel *dataModel;

@end

@implementation MKGTAdvBeaconController

- (void)dealloc {
    NSLog(@"MKGTAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTAdvBeaconController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTAdvBeaconModel alloc] init];
    }
    return _dataModel;
}

...
@implementation MKGTOTAPageModel

- (void)dealloc {
    NSLog(@"MKGTOTAPageModel销毁");
        ...
        if (self.otaType == 0) {
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKGTReceiveDeviceOTAResultNotification
                                                       object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKGTReceiveDeviceNpcOTAResultNotification
                                                       object:nil];
        }
...
- (BOOL)startOTA {
    ...
    [MKGTMQTTInterface gt_configOTAWithFilePath:self.filePath macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)startNpcOTA {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configNpcOTAWithFilePath:self.filePath macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (void)receiveOTAResult:(NSNotification *)note {
    ...
    if (self.receiveOTAResult) {
        self.receiveOTAResult(result);
    }
}

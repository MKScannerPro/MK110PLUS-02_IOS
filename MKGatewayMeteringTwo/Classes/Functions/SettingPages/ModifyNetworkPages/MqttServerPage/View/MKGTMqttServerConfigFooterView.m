//
//  MKGTMqttServerConfigFooterView.m
...
@implementation MKGTMqttServerConfigFooterViewModel
@end
...
@interface MKGTMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGTMqttServerSSLViewDelegate,
MKGTMqttServerSettingViewDelegate,
MKGTMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKGTMqttServerSSLView *sslView;

@property (nonatomic, strong)MKGTMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKGTMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKGTMqttServerConfigFooterView
...
#pragma mark - MKGTMqttServerSSLViewDelegate
- (void)gt_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)gt_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)gt_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKGTMqttServerSettingViewDelegate
...
#pragma mark - MKGTMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKGTMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKGTMqttServerSSLViewModel *sslModel = [[MKGTMqttServerSSLViewModel alloc] init];
    ...
    MKGTMqttServerLwtViewModel *lwtModel = [[MKGTMqttServerLwtViewModel alloc] init];
...
- (MKGTMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGTMqttServerSSLView alloc] init];
...
- (MKGTMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKGTMqttServerLwtView alloc] init];
...
- (MKGTMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKGTMqttServerSettingView alloc] init];

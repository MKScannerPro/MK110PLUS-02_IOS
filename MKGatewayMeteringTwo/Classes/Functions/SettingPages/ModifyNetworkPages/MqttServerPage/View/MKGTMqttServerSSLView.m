...
@implementation MKGTMqttServerSSLViewModel
@end

@interface MKGTMqttServerSSLView ()<MKGTMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKGTMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKGTMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKGTMqttServerSSLTextField *clientCertView;

@end

@implementation MKGTMqttServerSSLView
...
#pragma mark - MKGTMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)gt_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate gt_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate gt_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate gt_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKGTMqttServerSSLTextFieldModel *caModel = [[MKGTMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKGTMqttServerSSLTextFieldModel *clientKeyModel = [[MKGTMqttServerSSLTextFieldModel alloc] init];
    ...
    MKGTMqttServerSSLTextFieldModel *clientModel = [[MKGTMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttServerSSLView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttServerSSLView", @"gt_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttServerSSLView", @"gt_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKGTMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKGTMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKGTMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKGTMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKGTMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKGTMqttServerSSLTextField alloc] init];

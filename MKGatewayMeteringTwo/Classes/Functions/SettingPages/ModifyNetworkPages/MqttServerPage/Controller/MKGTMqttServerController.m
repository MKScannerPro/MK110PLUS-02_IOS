...
@interface MKGTMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGTMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKGTMqttServerModel *dataModel;

@property (nonatomic, strong)MKGTMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKGTMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKGTMqttServerController

- (void)dealloc {
    NSLog(@"MKGTMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKGTReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKGTMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)mk_selectedServerParams:(NSString *)fileName {
    [MKGTExcelDataManager parseDeviceExcel:fileName
        ...
        MKGTMqttServerModel *model = [MKGTMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKGTMqttServerModel *model = [[MKGTMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttServerController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGTMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGTMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKGTMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGTMqttServerConfigFooterViewModel alloc] init];
    }

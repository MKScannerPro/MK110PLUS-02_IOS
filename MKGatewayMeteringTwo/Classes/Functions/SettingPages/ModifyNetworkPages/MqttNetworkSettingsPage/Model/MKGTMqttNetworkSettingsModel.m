//
//  MKGTMqttNetworkSettingsModel.m
...
@interface MKGTMqttNetworkSettingsModel ()
...
@implementation MKGTMqttNetworkSettingsModel
...
- (BOOL)readNetworkInfos {
    ...
    [MKGTMQTTInterface gt_readNetworkInfosWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkInfos {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_modifyNetworkInfos:self macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

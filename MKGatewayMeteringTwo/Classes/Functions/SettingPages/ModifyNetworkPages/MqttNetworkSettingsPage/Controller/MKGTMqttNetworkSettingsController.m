//
//  MKGTMqttNetworkSettingsController.m
...
@interface MKGTMqttNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTMqttNetworkSettingsModel *dataModel;

@end

@implementation MKGTMqttNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKGTMqttNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttNetworkSettingsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTMqttNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTMqttNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}

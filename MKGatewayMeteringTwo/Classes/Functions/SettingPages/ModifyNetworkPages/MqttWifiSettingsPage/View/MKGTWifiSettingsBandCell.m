...
@implementation MKGTWifiSettingsBandCellModel
@end

@interface MKGTWifiSettingsBandCell ()
...
@implementation MKGTWifiSettingsBandCell

+ (MKGTWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKGTWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_wifiSettingsBandCell_countryChanged:)]) {
            [self.delegate gt_wifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }

//
//  MKGTMqttWifiSettingsController.m
...
@property (nonatomic, strong)MKGTMqttWifiSettingsModel *dataModel;

@end

@implementation MKGTMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKGTMqttWifiSettingsController销毁");
}
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKGTReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMqttWifiSettingsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTFilterByUIDModel ()
...
- (BOOL)readFilterByUID {
    ...
    [MKGTMQTTInterface gt_readFilterByUIDWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByUID {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByUID:self.isOn namespaceID:self.namespaceID instanceID:self.instanceID macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

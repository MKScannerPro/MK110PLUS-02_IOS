...
@interface MKGTFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByUIDModel *dataModel;

@end

@implementation MKGTFilterByUIDController

- (void)dealloc {
    NSLog(@"MKGTFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByUIDController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByUIDModel alloc] init];
    }
    return _dataModel;
}

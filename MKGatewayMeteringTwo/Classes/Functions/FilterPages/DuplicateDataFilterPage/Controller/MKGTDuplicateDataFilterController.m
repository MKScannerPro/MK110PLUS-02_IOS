//
//  MKGTDuplicateDataFilterController.m
...
@interface MKGTDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTDuplicateDataFilterModel *dataModel;

@end

@implementation MKGTDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKGTDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTDuplicateDataFilterController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}

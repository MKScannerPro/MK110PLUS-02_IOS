//
//  MKGTFilterByAdvNameController.m
...
@interface MKGTFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByAdvNameModel *dataModel;

@end

@implementation MKGTFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKGTFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKGTFilterEditSectionHeaderViewModel *headerModel = [[MKGTFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByAdvNameController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}

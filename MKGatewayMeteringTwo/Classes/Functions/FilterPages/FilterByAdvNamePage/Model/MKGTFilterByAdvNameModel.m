...
- (BOOL)readFilterByName {
    ...
    [MKGTMQTTInterface gt_readFilterByADVNameWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByName:(NSArray <NSString *>*)nameList {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByADVName:nameList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

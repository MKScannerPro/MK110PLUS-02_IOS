...
@interface MKGTFilterByOtherModel ()
...
- (void)configWithRawDataList:(NSArray <MKGTFilterRawAdvDataModel *>*)list
...
- (BOOL)readFilterByOther {
    ...
    [MKGTMQTTInterface gt_readFilterOtherDatasWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByOther:(NSArray <MKGTFilterRawAdvDataModel *>*)list relationship:(mk_gt_filterByOther)relationship {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:list macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

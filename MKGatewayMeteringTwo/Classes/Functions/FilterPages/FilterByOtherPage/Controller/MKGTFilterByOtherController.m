...
@property (nonatomic, strong)MKGTFilterByOtherModel *dataModel;

@end

@implementation MKGTFilterByOtherController

- (void)dealloc {
    NSLog(@"MKGTFilterByOtherController销毁");
}
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        MKGTFilterRawAdvDataModel *model = [[MKGTFilterRawAdvDataModel alloc] init];
...
- (mk_gt_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_gt_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_gt_filterByOther_AB;
        }
        return mk_gt_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_gt_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_gt_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_gt_filterByOther_AOrBOrC;
        }
    }
    return mk_gt_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByOtherController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByOtherModel alloc] init];
    }
    return _dataModel;
}

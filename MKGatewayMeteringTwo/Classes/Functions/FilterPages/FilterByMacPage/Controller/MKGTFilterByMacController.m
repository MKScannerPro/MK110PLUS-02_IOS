...
@interface MKGTFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByMacModel *dataModel;

@end

@implementation MKGTFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByMacController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByMacModel alloc] init];
    }
    return _dataModel;
}

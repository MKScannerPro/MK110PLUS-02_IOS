...
@interface MKGTFilterByURLModel ()
...
- (BOOL)readFilterByURL {
    ...
    [MKGTMQTTInterface gt_readFilterByUrlWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByURL {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByURL:self.isOn url:self.url macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

...
@property (nonatomic, strong)MKGTFilterByURLModel *dataModel;

@end

@implementation MKGTFilterByURLController

- (void)dealloc {
    NSLog(@"MKGTFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByURLController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByURLModel alloc] init];
    }
    return _dataModel;
}

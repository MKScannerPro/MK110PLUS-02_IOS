//
//  MKGTFilterByNanoBeaconController.m
...
@interface MKGTFilterByNanoBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByNanoBeaconModel *dataModel;

@end

@implementation MKGTFilterByNanoBeaconController

- (void)dealloc {
    NSLog(@"MKGTFilterByNanoBeaconController销毁");
}
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithManufactureList:manufactureList sucBlock:^{
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    ...
    cellModel1.dataListIndex = self.dataModel.triggerType;
    [self.section1List addObject:cellModel1];
}

- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.manufactureList.count; i ++) {
        MKTextFieldCellModel *cellModel = [[MKTextFieldCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"ID %ld",(long)(i + 1)];
        cellModel.textPlaceholder = @"2 Bytes";
        cellModel.textFieldType = mk_hexCharOnly;
        cellModel.textFieldValue = self.dataModel.manufactureList[i];
        cellModel.maxLength = 4;
        [self.section2List addObject:cellModel];
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByNanoBeaconController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByNanoBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByNanoBeaconModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTFilterByNanoBeaconModel ()
...
- (BOOL)readFilterInformation {
    ...
    [MKGTMQTTInterface gt_readFilterByNanoBeaconWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterInformation:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByNanoBeacon:self.isOn advType:self.triggerType manufactureIDList:list macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

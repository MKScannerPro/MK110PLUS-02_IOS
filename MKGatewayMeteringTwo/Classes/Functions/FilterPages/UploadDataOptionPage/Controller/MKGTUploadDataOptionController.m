//
//  MKGTUploadDataOptionController.m
...
@interface MKGTUploadDataOptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTUploadDataOptionModel *dataModel;

@end

@implementation MKGTUploadDataOptionController

- (void)dealloc {
    NSLog(@"MKGTUploadDataOptionController销毁");
}
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        self.dataModel.rawData_advertising = isOn;
        return;
    }
    if (index == 2) {
        //RAW Data-Response
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTUploadDataOptionController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTUploadDataOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTUploadDataOptionModel alloc] init];
    }
    return _dataModel;
}

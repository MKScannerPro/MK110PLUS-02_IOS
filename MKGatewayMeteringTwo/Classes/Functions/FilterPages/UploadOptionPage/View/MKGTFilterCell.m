...
@implementation MKGTFilterCellModel
@end

@interface MKGTFilterCell ()
...
@implementation MKGTFilterCell

+ (MKGTFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKGTFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGTFilterCellIdenty"];
    if (!cell) {
        cell = [[MKGTFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGTFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_filterValueChanged:index:)]) {
            [self.delegate gt_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTFilterCellModel.class]) {
        return;
    }

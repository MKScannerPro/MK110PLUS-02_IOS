...
@interface MKGTUploadOptionV2Model ()
...
@implementation MKGTUploadOptionV2Model
...
- (void)configDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        ...
        if (![self configFilterPHY]) {
            [self operationFailedBlockWithMsg:@"Config Filter PHY Error" block:failedBlock];
            return;
        }
...
- (BOOL)readDuplicateDataFilter {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_readDuplicateDataFilterDatasWithMacAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configDuplicateDataFilter:self.rule macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

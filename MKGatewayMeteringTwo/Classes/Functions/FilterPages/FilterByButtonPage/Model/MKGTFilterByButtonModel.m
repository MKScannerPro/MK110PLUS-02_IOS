...
@interface MKGTFilterByButtonModel ()
...
- (BOOL)readFilterByBXPButton {
    ...
    [MKGTMQTTInterface gt_readFilterBXPButtonWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByBXPButton {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterBXPButton:self macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

...
@interface MKGTFilterByTofModel ()
...
- (BOOL)readFilterByTof {
    ...
    [MKGTMQTTInterface gt_readFilterByTofWithMacAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTof:(NSArray <NSString *>*)codeList {
    __block BOOL success = NO;
    [MKGTMQTTInterface gt_configFilterByTofList:codeList isOn:self.isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

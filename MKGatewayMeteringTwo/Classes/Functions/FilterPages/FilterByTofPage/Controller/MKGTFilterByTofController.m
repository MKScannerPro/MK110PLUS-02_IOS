...
@interface MKGTFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByTofModel *dataModel;

@end

@implementation MKGTFilterByTofController

- (void)dealloc {
    NSLog(@"MKGTFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByTofController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByTofModel alloc] init];
    }
    return _dataModel;
}

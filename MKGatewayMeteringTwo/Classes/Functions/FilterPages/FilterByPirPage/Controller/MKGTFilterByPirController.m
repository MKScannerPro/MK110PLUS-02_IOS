...
@interface MKGTFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByPirModel *dataModel;

@end

@implementation MKGTFilterByPirController

- (void)dealloc {
    NSLog(@"MKGTFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByPirController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByPirModel alloc] init];
    }
    return _dataModel;
}

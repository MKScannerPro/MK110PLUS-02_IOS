//
//  MKGTFilterByRawDataV2Controller.m
...
@interface MKGTFilterByRawDataV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByRawDataV2Model *dataModel;

@end

@implementation MKGTFilterByRawDataV2Controller

- (void)dealloc {
    NSLog(@"MKGTFilterByRawDataV2Controller销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKGTFilterByBeaconController *vc = [[MKGTFilterByBeaconController alloc] init];
        vc.pageType = mk_gt_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKGTFilterByUIDController *vc = [[MKGTFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKGTFilterByURLController *vc = [[MKGTFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKGTFilterByTLMController *vc = [[MKGTFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKGTFilterByButtonController *vc = [[MKGTFilterByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP-Tag
        MKGTFilterByTagController *vc = [[MKGTFilterByTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //PIR
        MKGTFilterByPirController *vc = [[MKGTFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 3) {
        //MK TOF
        MKGTFilterByTofController *vc = [[MKGTFilterByTofController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 4) {
        //Nano Beacon
        MKGTFilterByNanoBeaconController *vc = [[MKGTFilterByNanoBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 5) {
        //Other
        MKGTFilterByOtherController *vc = [[MKGTFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configFilterBXPDeviceInfo:(BOOL)isOn {
    ...
    [MKGTMQTTInterface gt_configFilterFilterBXPDeviceInfo:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPACC:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_configFilterBXPAcc:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGTMQTTInterface gt_configFilterBXPTH:isOn macAddress:[MKGTDeviceModeManager shared].macAddress topic:[MKGTDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (MKGTFilterByRawDataV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByRawDataV2Model alloc] init];
    }
    return _dataModel;
}

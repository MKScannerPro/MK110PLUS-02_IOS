...
@interface MKGTFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTFilterByTagModel *dataModel;

@end

@implementation MKGTFilterByTagController

- (void)dealloc {
    NSLog(@"MKGTFilterByTagController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-Tag";
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTFilterByTagController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTFilterByTagModel alloc] init];
    }
    return _dataModel;
}

...
@interface MKGTPowerMeteringController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKGTPowerMeteringModel *dataModel;

@property (nonatomic, strong)UIButton *resetButton;

@end

@implementation MKGTPowerMeteringController

- (void)dealloc {
    NSLog(@"MKGTPowerMeteringController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Metering switch
        MKGTMeteringParamsController *vc = [[MKGTMeteringParamsController alloc] init];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receivePowerData:)
                                                 name:MKGTReceivePowerDataNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveEnergyData:)
                                                 name:MKGTReceiveEnergyDataNotification
                                               object:nil];
}
...
- (void)loadSection1Datas {
    ...
    cellModel1.leftIcon = LOADICON(@"MKGatewayMeteringTwo", @"MKGTPowerMeteringController", @"gt_electricityVoltageIcon.png");
    [self.section1List addObject:cellModel1];
    
    MKNormalTextCellModel *cellModel2 = [[MKNormalTextCellModel alloc] init];
    cellModel2.leftMsg = @"Current(mA)";
    cellModel2.rightMsg = self.dataModel.current;
    cellModel2.leftIcon = LOADICON(@"MKGatewayMeteringTwo", @"MKGTPowerMeteringController", @"gt_electricityCurrentIcon.png");
    [self.section1List addObject:cellModel2];
    
    MKNormalTextCellModel *cellModel3 = [[MKNormalTextCellModel alloc] init];
    cellModel3.leftMsg = @"Power(W)";
    cellModel3.rightMsg = self.dataModel.power;
    cellModel3.leftIcon = LOADICON(@"MKGatewayMeteringTwo", @"MKGTPowerMeteringController", @"gt_electricityPowerIcon.png");
    ...
    cellModel4.leftIcon = LOADICON(@"MKGatewayMeteringTwo", @"MKGTPowerMeteringController", @"gt_electricityEnergyIcon.png");
...
- (MKGTPowerMeteringModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTPowerMeteringModel alloc] init];
    }
    return _dataModel;
}

//
//  MKGTMeteringParamsController.m
...
@interface MKGTMeteringParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTMeteringParamsModel *dataModel;

@end

@implementation MKGTMeteringParamsController

- (void)dealloc {
    NSLog(@"MKGTMeteringParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMeteringParamsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTMeteringParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTMeteringParamsModel alloc] init];
    }
    return _dataModel;
}

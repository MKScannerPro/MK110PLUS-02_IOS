//
//  MKGTBleNTPTimezoneController.m
...
@interface MKGTBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKGTBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKGTBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleNTPTimezoneController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}

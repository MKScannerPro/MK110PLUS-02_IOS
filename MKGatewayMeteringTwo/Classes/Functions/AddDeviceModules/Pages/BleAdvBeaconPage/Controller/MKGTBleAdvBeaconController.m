...
@interface MKGTBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleAdvBeaconModel *dataModel;

@end

@implementation MKGTBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKGTBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleAdvBeaconController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}

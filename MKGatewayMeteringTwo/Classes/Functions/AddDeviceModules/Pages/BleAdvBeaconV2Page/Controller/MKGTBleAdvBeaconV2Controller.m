//
//  MKGTBleAdvBeaconV2Controller.m
...
@interface MKGTBleAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleAdvBeaconV2Model *dataModel;

@end

@implementation MKGTBleAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKGTBleAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleAdvBeaconV2Controller", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBleAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}

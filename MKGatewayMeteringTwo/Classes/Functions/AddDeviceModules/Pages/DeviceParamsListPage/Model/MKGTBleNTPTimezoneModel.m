...
@interface MKGTBleNTPTimezoneModel ()
...
@implementation MKGTBleNTPTimezoneModel
...
- (BOOL)readNTPHost {
    ...
    [MKGTInterface gt_readNTPServerHostWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNTPHost {
    __block BOOL success = NO;
    [MKGTInterface gt_configNTPServerHost:self.ntpHost sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTimezone {
    __block BOOL success = NO;
    [MKGTInterface gt_readTimeZoneWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimezone {
    __block BOOL success = NO;
    [MKGTInterface gt_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);

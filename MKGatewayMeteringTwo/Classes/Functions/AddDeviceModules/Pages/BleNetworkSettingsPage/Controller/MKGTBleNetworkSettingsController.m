//
//  MKGTBleNetworkSettingsController.m
...
@interface MKGTBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleNetworkSettingsModel *dataModel;

@end

@implementation MKGTBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKGTBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleNetworkSettingsController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}

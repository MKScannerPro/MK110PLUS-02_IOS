...
@interface MKGTBleNetworkSettingsModel ()
...
@implementation MKGTBleNetworkSettingsModel
...
- (BOOL)readDHCPStatus {
    ...
    [MKGTInterface gt_readDHCPStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDHCPStatus {
    __block BOOL success = NO;
    [MKGTInterface gt_configDHCPStatus:self.dhcp sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readIpAddress {
    __block BOOL success = NO;
    [MKGTInterface gt_readNetworkIpInfosWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIpAddress {
    __block BOOL success = NO;
    [MKGTInterface gt_configIpAddress:self.ip
                                 mask:self.mask

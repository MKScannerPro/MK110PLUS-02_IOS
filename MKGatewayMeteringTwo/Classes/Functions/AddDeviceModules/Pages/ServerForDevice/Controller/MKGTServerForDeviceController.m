//
//  MKGTServerForDeviceController.m
...
@interface MKGTServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGTServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKGTServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKGTServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKGTServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKGTServerForDeviceController

- (void)dealloc {
    NSLog(@"MKGTServerForDeviceController销毁");
}
...
#pragma mark - MKGTServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)gt_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)gt_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)gt_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)gt_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)gt_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)gt_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)gt_selectedServerParams:(NSString *)fileName {
    [MKGTExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKGTServerForDeviceModel *model = [MKGTServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKGTDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKGTDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKGTDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKGTDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKGTDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKGTDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKGTDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKGTDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKGTExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKGTServerForDeviceModel *emptyModel = [[MKGTServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTServerForDeviceController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGTServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGTServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKGTServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGTServerConfigDeviceFooterViewModel alloc] init];
    }

//
//  MKGTServerConfigDeviceFooterView.m
...
@implementation MKGTServerConfigDeviceFooterViewModel
@end
...
@interface MKGTServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGTMQTTSSLForDeviceViewDelegate,
MKGTServerConfigDeviceSettingViewDelegate,
MKGTMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKGTMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKGTMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKGTServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKGTServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gt_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate gt_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKGTMQTTSSLForDeviceViewDelegate
- (void)gt_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gt_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)gt_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate gt_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)gt_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)gt_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)gt_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKGTServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)gt_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate gt_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKGTMQTTLWTForDeviceViewDelegate
- (void)gt_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gt_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)gt_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gt_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)gt_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate gt_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)gt_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)gt_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gt_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKGTServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKGTMQTTSSLForDeviceViewModel *sslModel = [[MKGTMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKGTMQTTLWTForDeviceViewModel *lwtModel = [[MKGTMQTTLWTForDeviceViewModel alloc] init];
...
- (MKGTMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGTMQTTSSLForDeviceView alloc] init];
...
- (MKGTMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKGTMQTTLWTForDeviceView alloc] init];
...
- (MKGTServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKGTServerConfigDeviceSettingView alloc] init];

...
@implementation MKGTMQTTSSLForDeviceViewModel
@end

@interface MKGTMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKGTMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate gt_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate gt_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate gt_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate gt_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gt_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate gt_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGTMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForDeviceView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForDeviceView", @"gt_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTSSLForDeviceView", @"gt_switchUnselectedIcon.png") forState:UIControlStateNormal];

//
//  MKGTServerConfigDeviceSettingView.m
...
@interface MKGTServerConfigDeviceSettingView ()
...
@implementation MKGTServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gt_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gt_mqtt_deviecSetting_fileButtonPressed:1];
    }
}

...
@implementation MKGTMQTTLWTForDeviceViewModel
@end

@interface MKGTMQTTLWTForDeviceView ()
...
@implementation MKGTMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(gt_lwt_statusChanged:)]) {
        [self.delegate gt_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchSelectedIcon.png") : LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(gt_lwt_retainChanged:)]) {
        [self.delegate gt_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(gt_lwt_qosChanged:)]) {
        [self.delegate gt_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGTMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGTMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTMQTTLWTForDeviceView", @"gt_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gt_lwt_topicChanged:)]) {
                [self.delegate gt_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gt_lwt_payloadChanged:)]) {
                [self.delegate gt_lwt_payloadChanged:text];
            }
        };

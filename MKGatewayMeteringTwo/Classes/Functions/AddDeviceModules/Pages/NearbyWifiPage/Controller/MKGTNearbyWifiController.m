...
@interface MKGTNearbyWifiController ()<UITableViewDelegate,
UITableViewDataSource,
mk_gt_centralManagerScanWifiDelegate>
...
@implementation MKGTNearbyWifiController

- (void)dealloc {
    NSLog(@"MKGTNearbyWifiController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGTNearbyWifiCellModel *cellModel = self.dataList[indexPath.row];
    if ([self.delegate respondsToSelector:@selector(gt_nearbyWifiController_selectedWifi:)]) {
        [self.delegate gt_nearbyWifiController_selectedWifi:cellModel.ssid];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGTNearbyWifiCell *cell = [MKGTNearbyWifiCell initCellWithTableView:tableView];
...
- (void)parseContentList {
    ...
    while (index < length) {
        ...
        // 创建模型对象
        MKGTNearbyWifiCellModel *model = [[MKGTNearbyWifiCellModel alloc] init];
        model.bssid = bssid;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTNearbyWifiController", @"gt_refreshWifiListIcon.png") forState:UIControlStateNormal];

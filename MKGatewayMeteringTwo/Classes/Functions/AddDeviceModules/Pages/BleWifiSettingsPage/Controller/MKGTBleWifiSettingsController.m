//
//  MKGTBleWifiSettingsController.m
...
@interface MKGTBleWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleWifiSettingsModel *dataModel;

@end

@implementation MKGTBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKGTBleWifiSettingsController销毁");
}
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKGTDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKGTDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (MKGTBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}

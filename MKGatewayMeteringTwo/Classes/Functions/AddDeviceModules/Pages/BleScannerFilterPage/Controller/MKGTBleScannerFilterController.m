//
//  MKGTBleScannerFilterController.m
...
@interface MKGTBleScannerFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGTBleScannerFilterModel *dataModel;

@end

@implementation MKGTBleScannerFilterController

- (void)dealloc {
    NSLog(@"MKGTBleScannerFilterController销毁");
}
...
- (void)mk_gt_filterNormalTextFieldValueChanged:(NSString *)text index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKGTFilterNormalTextFieldCellModel *cellModel = self.section1List[1];
        cellModel.textFieldValue = text;
        return;
    }
}
...
- (void)loadSection1Datas {
    ...
    MKGTFilterNormalTextFieldCellModel *cellModel2 = [[MKGTFilterNormalTextFieldCellModel alloc] init];
    cellModel2.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMeteringTwo", @"MKGTBleScannerFilterController", @"gt_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGTBleScannerFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGTBleScannerFilterModel alloc] init];
    }
    return _dataModel;
}

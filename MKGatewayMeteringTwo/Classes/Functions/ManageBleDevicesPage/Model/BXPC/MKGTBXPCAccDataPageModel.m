...
@implementation MKGTBXPCAccDataPageModel

- (void)dealloc {
    NSLog(@"MKGTBXPCAccDataPageModel销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
}

- (instancetype)init {
    if (self = [super init]) {
        _mailFileName = @"BXP-C AccData.txt";
    }
    return self;
}
...
- (void)notifyAccData:(BOOL)isOn
    ...
    [MKGTMQTTInterface gt_bxpBXPCNotifyAccDataWithBleMac:[MKGTManageBleDevicesManager shared].bleMac
                                                  notify:isOn

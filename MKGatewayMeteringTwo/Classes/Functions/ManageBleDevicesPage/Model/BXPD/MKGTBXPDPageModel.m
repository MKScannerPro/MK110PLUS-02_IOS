...
@implementation MKGTBXPDPageModel

- (void)dealloc {
    NSLog(@"MKGTBXPDPageModel销毁");
...
- (id <MKScannerBXPDAccParamsProtocol>)accParamsProtocol {
    return [[MKGTBXPDAccParamsModel alloc] init];
}

- (id <MKScannerAccDataProtocol>)accProtocol {
    return [[MKGTBXPDAccDataPageModel alloc] init];
}

- (id <MKScannerBXPDAdvParamsProtocol>)advProtocol {
    return [[MKGTBXPDAdvParamsModel alloc] init];
}

- (void)readConnectedStatusWithSucBlock:(void (^)(id returnData))sucBlock
                            failedBlock:(void (^)(NSError *error))failedBlock {
    [MKGTMQTTInterface gt_readBXPDConnectedStatusWithBleMacAddress:[MKGTManageBleDevicesManager shared].bleMac
...
- (void)powerOffWithSucBlock:(void (^)(id returnData))sucBlock
                 failedBlock:(void (^)(NSError *error))failedBlock {
    [MKGTMQTTInterface gt_bxpBXPDPowerOffWithBleMac:[MKGTManageBleDevicesManager shared].bleMac
                                         macAddress:[MKScannerDeviceModelManager shared].macAddress
                                              topic:[MKScannerDeviceModelManager shared].subscribedTopic
                                           sucBlock:sucBlock
                                        failedBlock:failedBlock];

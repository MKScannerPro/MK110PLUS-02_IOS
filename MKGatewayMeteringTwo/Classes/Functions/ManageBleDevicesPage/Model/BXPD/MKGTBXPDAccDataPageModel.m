...
@implementation MKGTBXPDAccDataPageModel

- (void)dealloc {
    NSLog(@"MKGTBXPDAccDataPageModel销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
}

- (instancetype)init {
    if (self = [super init]) {
        _mailFileName = @"BXP-D AccData.txt";
    }
    return self;
}
...
- (void)notifyAccData:(BOOL)isOn
    ...
    [MKGTMQTTInterface gt_bxpBXPDNotifyAccDataWithBleMac:[MKGTManageBleDevicesManager shared].bleMac
                                                  notify:isOn

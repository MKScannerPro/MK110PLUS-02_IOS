//
//  MKGTBXPButtonAccDataPageModel.m
...
@implementation MKGTBXPButtonAccDataPageModel

- (void)dealloc {
    NSLog(@"MKGTBXPButtonAccDataPageModel销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
}

- (instancetype)init {
    if (self = [super init]) {
        _mailFileName = @"BXP-B-D AccData.txt";
    }
    return self;
}
...
- (void)notifyAccData:(BOOL)isOn
    ...
    [MKGTMQTTInterface gt_bxpBtnNotifyAccDataWithBleMac:[MKGTManageBleDevicesManager shared].bleMac
                                                 notify:isOn

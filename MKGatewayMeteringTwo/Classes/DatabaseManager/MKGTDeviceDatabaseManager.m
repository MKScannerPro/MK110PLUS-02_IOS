...
@implementation MKGTDeviceDatabaseManager
...
+ (void)insertDeviceList:(NSArray <MKGTDeviceModel *>*)deviceList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GTDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        for (MKGTDeviceModel *device in deviceList) {
            BOOL exist = NO;
...
+ (void)readLocalDeviceWithSucBlock:(void (^)(NSArray <MKGTDeviceModel *> *deviceList))sucBlock
                        failedBlock:(void (^)(NSError *error))failedBlock {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"GTDeviceDB")];
    if (![db open]) {
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GTDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        while ([result next]) {
            MKGTDeviceModel *dataModel = [[MKGTDeviceModel alloc] init];

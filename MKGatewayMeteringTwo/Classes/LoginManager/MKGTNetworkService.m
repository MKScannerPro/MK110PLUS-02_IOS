...
@implementation MKGTCreatScannerProDeviceModel
...
@interface MKGTNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKGTNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKGTNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKGTNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKGTCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKGTCreatScannerProDeviceModel *deviceModel in deviceList) {

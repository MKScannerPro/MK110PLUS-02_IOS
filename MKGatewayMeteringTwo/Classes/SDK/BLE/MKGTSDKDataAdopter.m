...
@implementation MKGTSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_gt_wifiSecurity)security {
    switch (security) {
        case mk_gt_wifiSecurity_personal:
            return @"00";
        case mk_gt_wifiSecurity_enterprise:
            return @"01";
    }
}

+ (NSString *)fetchWifiEapType:(mk_gt_eapType)eapType {
    switch (eapType) {
        case mk_gt_eapType_peap_mschapv2:
            return @"00";
        case mk_gt_eapType_ttls_mschapv2:
            return @"01";
        case mk_gt_eapType_tls:
            return @"02";
    }
}

+ (NSString *)fetchConnectModeString:(mk_gt_connectMode)mode {
    switch (mode) {
        case mk_gt_connectMode_TCP:
            return @"00";
        case mk_gt_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_gt_connectMode_CACertificate:
            return @"02";
        case mk_gt_connectMode_SelfSignedCertificates:
            return @"03";
    }
}

+ (NSString *)fetchMqttServerQosMode:(mk_gt_mqttServerQosMode)mode {
    switch (mode) {
        case mk_gt_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_gt_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_gt_mqttQosLevelExactlyOnce:
            return @"02";
    }
}

...
static NSString *const mk_gt_logName = @"mk_gt_bleLog";

NSString *const mk_gt_peripheralConnectStateChangedNotification = @"mk_gt_peripheralConnectStateChangedNotification";
NSString *const mk_gt_centralManagerStateChangedNotification = @"mk_gt_centralManagerStateChangedNotification";

NSString *const mk_gt_deviceDisconnectTypeNotification = @"mk_gt_deviceDisconnectTypeNotification";

static MKGTCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKGTCentralManager ()
...
@property (nonatomic, assign)mk_gt_centralConnectStatus connectStatus;

@end

@implementation MKGTCentralManager

- (void)dealloc {
    [self logToLocal:@"MKGTCentralManager销毁"];
    NSLog(@"MKGTCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKGTCentralManager初始化"];
...
+ (MKGTCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKGTCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_gt_receiveDevice:)]) {
                [self.delegate mk_gt_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_gt_startScan)]) {
        [self.delegate mk_gt_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_gt_stopScan)]) {
        [self.delegate mk_gt_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_gt_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_gt_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_gt_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_gt_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_gt_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_gt_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_gt_deviceDisconnectTypeNotification
                                                            object:nil
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA03"]]) {
        if ([[string substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"ee"]) {
            ...
            if ([cmd isEqualToString:@"51"]) {
                //扫描的wifi数据
                if ([self.wifiDelegate respondsToSelector:@selector(mk_gt_receiveWifi:)]) {
                    [self.wifiDelegate mk_gt_receiveWifi:[string substringFromIndex:6]];
                }
                return;
            }
        }
...
- (mk_gt_centralManagerStatus )centralStatus {
    ...
    ? mk_gt_centralManagerStatusEnable
    : mk_gt_centralManagerStatusUnable;
}

- (void)startScan {
    [[MKBLEBaseCentralManager shared] scanForPeripheralsWithServices:@[[CBUUID UUIDWithString:@"AA0E"]] options:nil];
}
...
- (void)addTaskWithTaskID:(mk_gt_taskOperationID)operationID
    ...
    MKGTOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_gt_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKGTOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKGTPeripheral *trackerPeripheral = [[MKGTPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKGTOperation *operation = [[MKGTOperation alloc] initOperationWithID:mk_gt_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.gt_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_gt_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_gt_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKGTOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_gt_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKGTOperation <MKBLEBaseOperationProtocol>*operation = [[MKGTOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKGTOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_gt_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_gt_logName dataList:@[string]];
}

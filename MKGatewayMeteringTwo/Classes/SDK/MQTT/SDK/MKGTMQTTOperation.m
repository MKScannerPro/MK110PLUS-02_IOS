...
/**
 线程ID
 */
@property (nonatomic, assign)mk_gt_serverOperationID operationID;
...
@implementation MKGTMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKGTMQTTOperation销毁");
}

- (instancetype)initOperationWithID:(mk_gt_serverOperationID)operationID
...
#pragma mark - MKGTMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKGTMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_gt_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_gt_defaultServerOperationID || operationID != self.operationID) {
        return;
    }

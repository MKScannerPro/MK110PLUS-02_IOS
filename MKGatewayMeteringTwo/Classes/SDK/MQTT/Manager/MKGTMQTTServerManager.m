...
static MKGTMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKGTMQTTServerManager)

@end

@implementation NSObject (MKGTMQTTServerManager)
...
@interface MKGTMQTTServerManager ()

@property (nonatomic, assign)MKGTMQTTSessionManagerState state;

@property (nonatomic, strong)MKGTServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKGTServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKGTMQTTServerManager
...
+ (MKGTMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKGTMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKGTServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(gt_didReceiveMessage:onTopic:)]) {
                [protocol gt_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKGTServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(gt_didChangeState:)]) {
                [protocol gt_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKGTMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKGTServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKGTServerManagerProtocol)]
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_gt_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_gt_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_gt_mqtt_recordDate"];
...
#pragma mark - getter
- (MKGTServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKGTServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKGTServerManagerProtocol>> *)managerList {
    if (!_managerList) {
